...
class Minvusage extends Mcrud_tablemeta
{
    function list($filter = null, $limit = null, $offset = null, $orderby = null) {
        ...
        $this->db->select("i.`usagerequestnum` as `usagerequestid_label`");
        ...
        $this->db->join("tcg_usagerequest i", "a.`usagerequestid`=i.`usagerequestid` AND i.`is_deleted`=0", "LEFT OUTER");
        $this->db->join("tcg_usagerequestitem j", "a.`usagerequestitemid`=j.`usagerequestitemid` AND j.`is_deleted`=0", "LEFT OUTER");
        ...
        if (!empty($itemtypeid)) {
            $this->db->where("h.typeid", $itemtypeid);
        }
        if (!empty($siteid)) {
    ...
    protected function __update_custom_column($temp_table_name) {
        ...
        $sql = "
        update " .$temp_table_name. " set usagerequestnum=usagerequestid, usagerequestlinenum=usagerequestitemid;
        ";
        $this->db->query($sql);
    }
    ...
    protected function __get_custom_column($columns) {
        ...
        for($i=0; $i<count($columns); $i++) {
            if ($columns[$i]['name'] == 'usagerequestnum') {
                $columns[$i]['visible'] = 1;
                $columns[$i]['allow_insert'] = 1;
                $columns[$i]['allow_edit'] = 1;
            }
            else if ($columns[$i]['name'] == 'usagerequestlinenum') {
                $columns[$i]['visible'] = 1;
                $columns[$i]['allow_insert'] = 1;
                $columns[$i]['allow_edit'] = 1;
            }
