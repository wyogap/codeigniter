...
<script type="text/javascript">
    ...
    function onclick_approve(rowIdx, dt, id) {
        let row = dt.row("#"+id);
        let data = row.data();
        let label = data['usagerequestnum'];
        ...
        $.confirm({
            title: 'Konfirmasi',
            content: 'Setujui Perintah Distribusi ' +label+ '?',
            closeIcon: true,
            columnClass: 'medium',
            buttons: {
                cancel: {
                ...
                confirm: {
                    ...
                    action: function(){
                        $.ajax({
                            url: "{$site_url}disbekal/wfinventory/approveur",
                            type: 'POST',
                            data: { id: id },
                            dataType: 'json',
                            ...
                            success: function(response) {
                                if (response.status === null || response.status == 0 || (response.error !== undefined && response.error !== null && response.error !== '')) {
                                    ...
                                    toastr.error("Tidak berhasil menyetujui Perintah Distribusi " +label)
                                    toastr.error(msg)
                                    return;
                                }
                                ...
                                toastr.success("Berhasil menyetujui Perintah Distribusi " +label+ ".");

                                onselect_perintahdistribusi(dt, null, null);
                            },
    ...
    function onclick_close(rowIdx, dt, id) {
        ...
        $.confirm({
            title: 'Konfirmasi',
            content: 'Arsipkan Perintah Distribusi ' +label+ '?',
            closeIcon: true,
            columnClass: 'medium',
            buttons: {
                cancel: {
                ...
                confirm: {
                    ...
                    action: function(){
                        $.ajax({
                            url: "{$site_url}disbekal/wfinventory/closeur",
                            type: 'POST',
                            data: { id: id },
                            dataType: 'json',
                            // IMPORTANT: DO NOT set content-type to JSON. It is not supported well by codeigniter
                            ...
                            success: function(response) {
                                if (response.status === null || response.status == 0 || (response.error !== undefined && response.error !== null && response.error !== '')) {
                                    ...
                                    toastr.error("Tidak berhasil mengarsipkan Perintah Distribusi " +label)
                                    toastr.error(msg)
                                    return;
                                }
                                ...
                                toastr.success("Berhasil mengarsipkan Perintah Distribusi " +label+ ".");
